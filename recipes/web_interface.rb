...
remote_file "download_web_interface" do
  ...
  source "#{node[:graylog2][:repo]}/graylog2-web-interface/releases/download/#{node[:graylog2][:web_interface][:version]}/graylog2-web-interface-#{node[:graylog2][:web_interface][:version]}.tar.gz"
...
execute "sudo chown -R #{node[:apache][:user]}:#{node[:apache][:group]} graylog2-web-interface-#{node[:graylog2][:web_interface][:version]}" do
  cwd "#{node[:graylog2][:basedir]}/rel"
  not_if do
